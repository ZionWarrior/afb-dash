...
def build_html(summary, rows, chart):
    ...
    for r in rows:
        ...
        if r.get("is_manual"):
            # Anleihe/Sparbrief: Nennwert + Kurse in Prozent des Nennwerts
            menge_cell = f"{fmt(r['nominal'])}&thinsp;\u20ac nom."
            entry_cell = f"{fmt(r['entry_pct'])}&thinsp;%"
            kurs_cell = f"{fmt(r['price_pct'])}&thinsp;%"
        else:
            menge_cell = qty_fmt(r["qty"])
            entry_cell = f"{fmt(r['avg_entry'], entry_dec)}&thinsp;{sym}"
            kurs_cell = f"{fmt(r['last'], entry_dec)}&thinsp;{sym}"
        # Wert: native Waehrung und EUR in einer Spalte
        if is_eur:
            wert_cell = f"{fmt(r['value_eur'])}&thinsp;\u20ac"
        else:
            wert_cell = f"{fmt(r['value_nat'])}&thinsp;{sym}&thinsp;/&thinsp;{fmt(r['value_eur'])}&thinsp;\u20ac"
        zins_cell = (
...
<style>
  ...
  * {{ margin: 0; padding: 0; box-sizing: border-box; }}
  ...
  h2 {{ font-family: "IBM Plex Sans", system-ui, sans-serif; font-weight: 500; font-size: 15px;
  ...
  .tick {{ display: inline-block; font-family: "IBM Plex Mono", monospace; font-size: 11px;
           ...
           padding: 2px 0; margin-right: 12px; width: 72px; text-align: center; }}
...
const chart = new Chart(ctx, {{
  ...
  options: {{
    ...
    scales: {{
      ...
      y: {{
        grid: {{ color: "#141518" }},
        afterFit: axis => {{ axis.width = Math.max(axis.width, 76); }},
        ticks: {{
